...
public interface UserService {
     ...
     * @since 1.0
     */
    Boolean checkSignUpStatus(User currentUser);

    /**
     * 회원가입 폼에 바인딩할 데이터를 조회합니다.
     * 조회한 사용자 정보를 기반으로 수정 가능한 데이터만 반환합니다.
     *
     * @param currentUser 인증된 사용자 객체
     ...
     * @since 1.0
     */
    SignUpFormDTO getSignUpFormData(User currentUser);
     ...
     * @since 1.0
     */
    void updateRequiredInfo(User currentUser, SignUpDTO additionalInfo);

    /**
     * 회원 로그인을 처리합니다.
     * 로그인에 성공할 경우 인증 토큰을 반환합니다.
     *
     * @param currentUser 로그인 요청 회원
     * @return 인증 토큰
     *
     * @since 1.0
     */
    String signIn(User currentUser);
}
